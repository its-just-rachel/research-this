...
| POST | `/v1/advisories` | Create a draft advisory | Analyst | Creates advisory in `assembling` state. Body includes title, classification references, linked tech profile ID, content. |
| POST | `/v1/advisories/{id}/submit-for-review` | Submit advisory for editorial review | Analyst | Transitions from `assembling` to `review`. Precondition: at least one linked tech profile required. |
...
|---|---|---|---|
| GET | /v1/market-reports | viewer+ | List market reports. Filters: state, topic_scope_type, topic_scope_ref, is_active. |
| POST | /v1/market-reports | senior_analyst+ | Create a new market report in `draft` state. |
| GET | /v1/market-reports/{id} | viewer+ | Get a single market report including current predictions. |
| PATCH | /v1/market-reports/{id} | senior_analyst+ | Update draft or under_review market report fields. |
| POST | /v1/market-reports/{id}/submit-for-review | senior_analyst+ | Transition `draft → under_review`. Creates peer_review queue entry. |
| POST | /v1/market-reports/{id}/publish | architect+ | Transition `under_review → published`. Sets published_at. Notifies watchers. |
| POST | /v1/market-reports/{id}/withdraw | architect+ | Transition `published → withdrawn`. |
| POST | /v1/market-reports/{id}/new-version | senior_analyst+ | Create a new version. Old record transitions to `superseded`. New record starts in `draft`. |
| GET | /v1/market-reports/{id}/versions | viewer+ | List all versions of a market report lineage. |
| GET | /v1/market-reports/{id}/predictions | viewer+ | List predictions for a market report. |
| POST | /v1/market-reports/{id}/predictions | senior_analyst+ | Add a prediction to a market report. |
| PATCH | /v1/market-reports/{id}/predictions/{pid} | architect+ | Update prediction resolution status and evidence. |

---

### 3.5 Match Sets (Classification)

<!-- formerly §3.4 — renumbered to accommodate §3.4 Market Reports -->
...
### 3.6 Search, Watchlists, and Alerts
...
| GET | `/v1/watchlists/{id}` | Get a single watchlist by ID | Analyst | Returns watchlist record including filter_criteria and alert_conditions. |
| PATCH | `/v1/watchlists/{id}` | Update watchlist filter criteria or alert conditions | Analyst (owner only) | Update `filter_criteria` or `alert_conditions`. Only the owning user may update. |
| DELETE | `/v1/watchlists/{id}` | Deactivate a watchlist | Analyst (owner only) | Sets `is_active = false`. Does not delete the record. |
...
| POST | `/v1/alerts/{id}/mark-read` | Mark an alert as read | Analyst | Marks an alert as read; sets `read_at` timestamp and `read = true`. Idempotent: marking an already-read alert returns 200 without error. |
